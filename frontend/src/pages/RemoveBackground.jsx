  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={e => setInput(e.target.value)} type="file" accept='image/*' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600' required/>
        <p className='mt-1 text-xs font-extralight '>Supports JPG,PNG and other image formats</p>
        
        <br />
        <button type='submit' className='mx-auto flex items-center w-full justify-center gap-3 bg-gradient-to-r from-[#fc9c4f] to-[#ff3232] px-4 py-2 text-white rounded-lg text-xs sm:text-sm cursor-pointer'>
          <Eraser className='w-3 sm:w-4' />
          Remove background
        </button>
      </form>
      {/* right col */}
